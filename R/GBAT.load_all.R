...
GBAT.load_all <- function(){
	...
	## This is all original code GMC used to detect installed versions of geosupport rGBAT[version] packages:
	## Commenting out for standalone Linux version:
		...
		#invisible(suppressMessages(lapply(as.character(gc.pks$Packagf), require, character.only = TRUE, lib.loc = my_loc)))

	# Instead, manually create DataFrame hard-coded to geosupport version 24B, and return that for the rest of the code expecting it:
	gc.pks <- data.frame(Package = c('rGBAT24B'),
			     Version = c('24.2'),
			     Release = c('24B')
			     )
	rownames(gc.pks) <- 1:nrow(gc.pks)
