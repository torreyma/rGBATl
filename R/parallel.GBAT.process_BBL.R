...
#' @title Geocode an extremely large data frame containing a column of BBL with a user selected release or version of DCP's Geosupport.
#' @name parallel.GBAT.process_BBL
#' @aliases parallel.GBAT.process_BBL
#' @import parallel
#' @import data.table
#' @export parallel.GBAT.process_BBL
...
#' @usage parallel.GBAT.process_BBL(in_clus, in_df, bbl_col_name, 
...
#' @examples #create a large data frame of BBL
#' in_bbl <- 1001990021
...
parallel.GBAT.process_BBL <- function(in_clus,in_df,bbl_col_name,source_cols,geocode_fields,return_type = "all",GBAT_name){
	...
	iGBAT <- GBAT.check_fields(GBAT_name,c("FBL","all"),geocode_fields)
	...
	jf <- joined_fields[GC_fldname %in% new.geocode_fields & !(is.na(GC_join_fld1)) & !(is.na(GC_join_fld2)) & GC_function %in% c("FBL","all"),]
	...
	if(GBAT.status %in% 1:2) {
		...
		###export variables###
		parallel::clusterExport(clus, c("in_clus","in_df",'bbl_col_name',"source_cols","GBAT.package","new.geocode_fields","my_loc","chunk",'return_type'), envir=environment())
		...
		out_df <- 
			...
			return(data.table::rbindlist(lapply(1:max(in_df.p$pl), function(yy)
					get("process_BBL", envir = asNamespace(GBAT.package), inherits = FALSE)(in_df=in_df.p[pl==yy], bbl_col_name=bbl_col_name, source_cols=source_cols, geocode_fields=new.geocode_fields, return_type=return_type)
