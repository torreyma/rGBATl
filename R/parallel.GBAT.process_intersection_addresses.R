...
#' @title Geocode an extremely large data frame of NYC intersection addresses with a user selected release or version of DCP's Geosupport.
#' @name parallel.GBAT.process_intersection_addresses
#' @aliases parallel.GBAT.process_intersection_addresses
#' @import parallel
#' @import data.table
#' @export parallel.GBAT.process_intersection_addresses
#' @param in_clus the number of clusters available to the function as integer.  Required.
#' @param in_df a data frame containing NYC intersection addresses.  Required.
#' @param street1_col_name the name of the first street column as string.  Required.
#' @param street2_col_name the name of the second street column as string.  Required.
#' @param boro_code1_col_name the name of the first borough code column as string.  Required.
...
#' @param boro_code2_col_name the name of the second borough code column as string.  Optional.
#' @param com_dir_col_name the name of the compass direction column as string.  Optional.
...
#' @usage parallel.GBAT.process_intersection_addresses(in_clus, in_df,  
...
#' @examples #create a large data frame of intersection addresses
#' street1 <- paste(1:271,"ST",sep=" ")
#' street2 <- paste(rep(1:100,3),c(rep("AVE",each=100),
#'     rep("RD",each=100),rep("DR",each=100)),sep=" ")
...
#' #with five cores
#' system.time({ df_gc1 <- parallel.GBAT.process_intersection_addresses(5, df, 
#'     "street1", "street2", "boro", source_cols, geocode_fields, NULL, 
#'     NULL, "all", "18C")})
#' 
#' #with one core
#' system.time({ df_gc2 <- GBAT.process_intersection_addresses(df, 
#'     "street1", "street2", "boro", source_cols, geocode_fields, NULL, 
#'     NULL, "all", "18C")})

parallel.GBAT.process_intersection_addresses <- function(in_clus,in_df,street1_col_name,street2_col_name,boro_code1_col_name,source_cols,geocode_fields,boro_code2_col_name=NULL,com_dir_col_name=NULL,return_type = "all",GBAT_name){
	...
	iGBAT <- GBAT.check_fields(GBAT_name,c("F2","all"),geocode_fields)
	...
	jf <- joined_fields[GC_fldname %in% new.geocode_fields & !(is.na(GC_join_fld1)) & !(is.na(GC_join_fld2)) & GC_function %in% c("F2","all"),]
	...
	if(!is.DT) in_df <- data.table::as.data.table(in_df)
	...
	if(GBAT.status %in% 1:2) {
		...
		###export variables###
		parallel::clusterExport(clus, c("in_clus","in_df","street1_col_name","street2_col_name","boro_code1_col_name","source_cols","boro_code2_col_name","com_dir_col_name","return_type","GBAT.package","new.geocode_fields","my_loc","chunk"), envir=environment())
		...
		out_df <- 
			...
			return(data.table::rbindlist(lapply(1:max(in_df.p$pl), function(yy)
				get("process_intersection_addresses", envir = asNamespace(GBAT.package), inherits = FALSE)(in_df=in_df.p[pl==yy], street1_col_name=street1_col_name, street2_col_name=street2_col_name, boro_code1_col_name=boro_code1_col_name, source_cols=source_cols, geocode_fields=new.geocode_fields, boro_code2_col_name=boro_code2_col_name, com_dir_col_name=com_dir_col_name, return_type=return_type)
		...
		if (nrow(jf) > 0){
			...
			for(i in 1:nrow(jf_tbl)){
				#out_df <- merge(out_df,get(jf_tbl[i]$GC_table),by=jf_tbl[i]$GC_join_fld,all.x=TRUE)
