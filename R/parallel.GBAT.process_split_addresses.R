#' @description The \code{parallel.GBAT.process_split_addresses} function allows the user to select a release or version of DCP's Geosupport to more efficiently geocode large data frames (+10,000 records) of split NYC addresses where house number and street name are in separate columns.  It aggregates, duplicates, geocodes using parallel processing, and then merges the geocoded results back to the original data frame.  The \code{parallel.GBAT.process_split_addresses} function utilizes Geosupport functions 1A, 1E, and AP. 
#'
...
#' @title Geocode an extremely large data frame of split NYC addresses with a user selected release or version of DCP's Geosupport.
#' @name parallel.GBAT.process_split_addresses
#' @aliases parallel.GBAT.process_split_addresses
#' @import parallel
#' @import data.table
#' @export parallel.GBAT.process_split_addresses
...
#' @usage parallel.GBAT.process_split_addresses(in_clus, in_df, hse_num_col_name, 
#'     street_col_name, unit_col_name = NULL, third_col_name, source_cols, 
...
#' H_NUM <- c(1:5000,1:2400,1:3400,1:3400)
#' ST_NAME <- c(rep("BROADWAY",5000),rep("1 AVE",2400),
#'     rep("ATLANTIC AVE",3400), rep("FULTON ST",3400))
#' BORO_CODE <- ifelse(ST_NAME %in% c("ATLANTIC AVE","FULTON ST"),3,1)
#' u_id <- 1:length(ST_NAME)
#' df = data.frame(u_id, H_NUM, ST_NAME, BORO_CODE)
#'
...
parallel.GBAT.process_split_addresses <- function(in_clus,in_df,hse_num_col_name,street_col_name,unit_col_name = NULL,third_col_name,source_cols,geocode_fields,third_col_type,return_type = "all",GBAT_name){
	...
	if(GBAT.status %in% 1:2) {
		...
		###export variables###
		parallel::clusterExport(clus, c("in_clus","in_df",'hse_num_col_name','street_col_name','unit_col_name',"source_cols",'third_col_name',"GBAT.package","new.geocode_fields","my_loc","chunk",'third_col_type','unit.chk','return_type'), envir=environment())
		...
		out_df <- data.table::rbindlist(parallel::parLapply(clus, 1:in_clus, function(zz) {
			...
			if(unit.chk==3){
				...
				return(data.table::rbindlist(lapply(1:max(in_df.p$pl), function(yy)
					get("process_split_addresses", envir = asNamespace(GBAT.package), inherits = FALSE)(in_df=in_df.p[pl==yy], hse_num_col_name=hse_num_col_name, street_col_name=street_col_name, third_col_name=third_col_name, source_cols=source_cols, geocode_fields=new.geocode_fields, third_col_type=third_col_type, return_type=return_type)
				...
				###lapply chunks###
				return(data.table::rbindlist(lapply(1:max(in_df.p$pl), function(yy)
					get("process_split_addresses", envir = asNamespace(GBAT.package), inherits = FALSE)(in_df=in_df.p[pl==yy], hse_num_col_name=hse_num_col_name, street_col_name=street_col_name, unit_col_name=unit_col_name,third_col_name=third_col_name, source_cols=source_cols, geocode_fields=new.geocode_fields, third_col_type=third_col_type, return_type=return_type)
				), use.names=TRUE, fill=TRUE))
