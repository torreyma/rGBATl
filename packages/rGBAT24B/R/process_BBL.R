...
process_BBL <- function(in_df, bbl_col_name, source_cols, geocode_fields, return_type="all") {
	
	grc_fld <- "FBL.ret_code"

	ret_cols <- c(source_cols, geocode_fields)
	
	col_vec <- c(bbl_col_name)
	...
	GC_df <- rGBAT24B:::GBAT_BL_df(df_unq, "u_id", bbl_col_name, unique(c(geocode_fields,grc_fld)))
