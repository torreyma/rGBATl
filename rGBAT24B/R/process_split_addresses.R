...
process_split_addresses <- function(in_df, hse_num_col_name, street_col_name, unit_col_name=NULL, third_col_name, source_cols, geocode_fields, third_col_type="boro_code", return_type="all") {
	
	grc_fld <- "F1E.output.ret_code"

	ret_cols <- c(source_cols, geocode_fields)

	col_vec <- c(hse_num_col_name,street_col_name,third_col_name)
	
	if(!(is.null(unit_col_name))) col_vec <- c(col_vec,unit_col_name)
	
	in_df[,(col_vec):= lapply(.SD, as.character), .SDcols = col_vec]
	
	for (col in col_vec) in_df[is.na(get(col)), (col) := ""]
	...
	GC_df <- rGBAT24B:::GBAT_2_df(df_unq, "u_id", hse_num_col_name, street_col_name, third_col_name, third_col_type, unique(c(geocode_fields,grc_fld)), unit_col_name)
